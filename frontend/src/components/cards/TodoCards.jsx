...
const TodoCards = (props) => {
  ...
  return (
    <div className="p-10 flex gap-4 bg-pink-300">
      {isLoading ? (
        <h2>Loading...</h2>
      ) : error ? (
        ...
        data?.map((item) => (
          <div
            key={item.id}
            className="bg-sky-800 border border-sky-400 rounded-md px-6 py-5 flex justify-between items-center max-w-xs flex-wrap"
          >
            <div>
